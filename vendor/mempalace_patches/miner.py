...
def _build_drawer_metadata(
    ...
    # ── LIVING-MEMORY EXTENSION (galadriel, 2026-06-15; re-applied 2026-08-26 for 3.8.0) ──
    ...
    # ── LIVING-MEMORY EXTENSION (galadriel, 2026-06-15; re-applied 2026-08-26 for 3.8.0) ──
    # Lifecycle + provenance for verbatim drawers. ChromaDB rejects None
    # values in metadata, so only set nullable fields when present.
    metadata["lifecycle_status"] = lifecycle_status or "active"
    try:
        metadata["confidence"] = float(confidence)
    except (TypeError, ValueError):
        metadata["confidence"] = 1.0
    metadata["origin"] = origin or "unknown"
    if session_id:
...
def add_drawer(
    ...
    metadata = _build_drawer_metadata(
        ...
        lifecycle_status=lifecycle_status, confidence=confidence,
        origin=origin, session_id=session_id, superseded_by=superseded_by,
    )
    collection.upsert(
        documents=[content],
        ids=[drawer_id],
...
# =============================================================================
# LIVING-MEMORY EXTENSION (galadriel, 2026-06-15; re-applied 2026-08-26 for 3.8.0)
# front-matter directive parser
# =============================================================================


def _parse_living_memory_directives(content: str) -> dict:
    """Extract optional drawer-level provenance directives from a file's header.

    Recognises lines of the form ``- origin: decision``, ``- confidence: 0.6``,
    ``- session: <id>`` within the first ~30 lines (the front-matter zone).
    Returns a dict with any of ``origin``/``confidence``/``session_id`` found.
    Tolerant: malformed values are ignored, never raise. Absent -> empty dict
    (callers fall back to active/1.0/unknown defaults).
    """
    out: dict = {}
    for line in content.splitlines()[:30]:
        ...
        if low.startswith("origin:"):
            ...
            try:
                ...
                pass
        elif low.startswith("session:") or low.startswith("session_id:"):
...
def process_file(
    ...
    # ── LIVING-MEMORY EXTENSION (galadriel, 2026-06-15; re-applied 2026-08-26 for 3.8.0) ──
    # Read optional drawer-level directives from the file's header lines
    # ("- origin: ...", "- confidence: ...", "- session: ..."). Used by
    # agent-filed drawers (harness palace_add_drawer) to carry self-assessed
    # provenance through the mine. Absent directives -> defaults (active/1.0).
    _lm = _parse_living_memory_directives(content)
    # ── END LIVING-MEMORY EXTENSION ──
                ...
                for chunk in chunks[batch_start : batch_start + DRAWER_UPSERT_BATCH_SIZE]:
                    ...
                    batch_metas.append(
                        _build_drawer_metadata(
                            ...
                            # LIVING-MEMORY (galadriel 2026-06-15): thread parsed provenance.
                            confidence=_lm.get("confidence", 1.0),
                            origin=_lm.get("origin", "unknown"),
                            session_id=_lm.get("session_id"),
                        )
                    )
