...
echo ""
echo "🏰 Optional — seed the memory palace (MemPalace) for semantic recall:"
echo "  cp mempalace.yaml.example mempalace.yaml   # edit room layout if needed"
echo "  mempalace init                              # creates ~/.mempalace/"
echo "  mempalace mine .                            # indexes this repo"
echo "  sudo systemctl restart galadriel            # pick up the seeded palace"
echo ""
echo "  Without seeding, palace tools return [palace unavailable] and the rest"
echo "  of the harness runs normally. See README.md for the full explanation."
